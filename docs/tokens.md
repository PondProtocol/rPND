...
- **Config intends:** transferable, lockable, clawback off with `ImmutableFlags` requesting a permanent freeze
- **On ledger now:** nothing — no `MPTokenIssuance` exists from any Pond Protocol account, on any network
- **Once applied:** the freeze needs the DynamicMPT amendment, which mainnet does not have, so the create as configured is rejected there. See [`rpnd-spec.md`](rpnd-spec.md)
